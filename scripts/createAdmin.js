...
// Define the user schema (copied from your server.js)
const userSchema = new mongoose.Schema({
    ...
    // Add any other fields from your userSchema if needed
});
...
async function createAdmin() {
    try {
        ...
        const existingAdmin = await User.findOne({ email: 'admin@saxonbank.com' });
        if (existingAdmin) {
            console.log('❌ Admin user already exists. Use these credentials:');
        ...
        const accountNumber = 'ADM' + Math.floor(10000000 + Math.random() * 90000000).toString();
